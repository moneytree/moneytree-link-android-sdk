...
> :warning: The SDK requires Android 6 and above.
...
You might want to provide an easy way to allow the user to access their Moneytree account settings from within your app. The SDK provides the interface function `openSettings(Activity, String)` for that purpose.
...
> :warning: Please note that users may be required to re-authenticate in order to access Account Settings. This can be by re-entering their email and password or via Login Link, described below.

> :information_source: As described in [Passwordless Sign Up/Login and Login Link](#passwordless-sign-uplogin-and-login-link) you can perform settings navigation using Login Link.
