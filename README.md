...
The Moneytree LINK SDK is offered through Maven. Simply add the Moneytree Maven repository to your project configuration:
...
Then add the following dependencies in your `build.gradle` or `build.gradle.kts` file:
...
android {
    defaultConfig {
        manifestPlaceholders += mapOf(
            // For production use this instead
            // myaccount.getmoneytree.com
            "linkHost" to "myaccount-staging.getmoneytree.com",
            // clientIdShort: "<first 5 chars of your client ID>"
            "clientIdShort" to "abcde"
        )
    }
}
```

Replacing `clientIdShort` with the **first 5 characters of your Client ID**.
...
The `scopes(...)` function used in the `Builder` above is basically a set of permissions the user needs to provide to the accessing application to be able to get a user token. More on this in [Account Access Scopes](#account-access-scopes).
...
val authConfig = LinkAuthOptions
    ...
    .buildAuthorize("awesomeness@awesome.awe")
...
val authConfig = LinkAuthOptions
    ...
    .buildAuthorize("simply.awesome@awesome.awe")
...
val authConfig = LinkAuthOptions
    ...
    .buildOnboarding("awesomeness@awesome.awe") // Email is required for Onboarding
...
Login can be done through any of the normal auth flows. The user will be given the option to login through email link (Magic Link) or using email and password.

For requesting a Magic Link navigation use `requestMagicLink(String, String, Action)`, where the parameters are:
...
After you request a Magic Link flow you need to consume the incoming deep link.
...
To open the Moneytree Vault the SDK interface provides the `openVault(Activity, LinkRequestContext)` function, where:
...
MoneytreeLink.getInstance().openVault(
    ...
    LinkRequestContext.Builder()
        .userEmail("awesome.email@awesome.awe") // Used to pre-populate the login screen if not already logged in.
        .build()
)
```
...
```kotlin
MoneytreeLink.getInstance().openSettings(activity, "awe@awe.awe")
```

> :information_source: As described in [Onboarding and Magic Link](#onboarding-and-magic-link) you can perform settings navigation using Magic Links
    ...
    when {
        ...
        result is LinkResult.Error -> {
            ...
            if (error == LinkError.UNAUTHORIZED) {
                // Something blew up no token for you...
            } else {
                // Bad things!!!
            }
        }
...
with(MoneytreeLink.getInstance()) {
    ...
    onError(context) { e ->
        if (e.error == LinkError.UNAUTHORIZED) {
            ...
            // Bad things!!!
            // You message is here: e.error.message
        }
    }
}
```
...
We offer the ability to register your FCM token with our services. If you do so the users will be able to receive push notifications with useful information about their account.
...
with(MoneytreeLink.instance) {
    onPkceAuthorized(context) { token ->
        // Do stuff with your token.value
    ...
    // More of the callbacks here...
}
```

> :information_source: `registerFcmToken(...) is clever enough to do nothing is an old token is used again or unregister an old token if a new one is provided, so you do not need to care about that.

Similarly you can call `unregisterFcmToken(String, Action?)` when logging out to ensure no FCM token is still active.
